#!/usr/bin/env python

from __future__ import print_function

# Prime polinomial 
# f(x) = x8 + x4 + x3 + x2 + 1
f = 0x11d

# Powers of x
# Calculated with that simple code:
#
# p = 1
# POWERS.append(1)
# for a in range(1, 255):
#     p = mulx(p)
#     POWERS.append(p)

POWERS = [
,
]

def mulx(a):
    """
    Multiply element a by x by shifting 
    and reducing over prime polinomial
    """
    a <<= 1

    # Reduce if we've exceeded GF(2^8)
    if a & 0x100:
        a ^= f
    
    return a

def mul(a, b):
    """
    Multiply 2 polynomials.

    Multiplication is done by factorization as 
    a consecutive addition of multiplications
    of 'a' on 'x' on current highest bit.
    """

    bits = 7
    c = 0
    # Loop over 7 bits, last bit after loop
    while bits:
        c ^= a * ((b & 0x80) >> 7)
        c = mulx(c)
        b <<= 1; 
        bits -= 1

    # Last sum b0*a(x)
    c ^= a * ((b & 0x80) >> 7)
    
    return c

def pow(a):
    """
    Computes 'a' power of x.
    Actually just a simple lookup in precalculated table.
    """
    return POWERS[a]

def log(a):
    """
    Computes logarithm of 'a' base 'x'.
    Actually just a simple lookup in precalculated table.
    """
    for i, x in enumerate(POWERS):
        if x == a:
            return i

def inv(a):
    """ 
    Computes inversion of a.

    a^(-1) = a^(256 - 2) = a^254
    a = x^b, because x is a primitive element.
    then
    a^(-1) = (x^b)^(-1) = x^254b <=> x^(254b % 255)
    """

    b = log(a)
    inv_pow = 254*b % 255
    return pow(inv_pow)


def brute_force_primitive():
    """ Computes field primitive elements with brute-force """

    primitives = []

    # We don't check 0 because it's powers is always 0
    # We don't check 1 because it's powers is always 1
    # range(2, 256) gives 2..255
    for a in range(2, 256):
        b = log(a)
        elements = set()
        for i in range(2, 255):
            elements.add(pow((i*b) % 255))

        # 253 elements + 0 + 1 + a itself = 256 elements in GF(2^8)
        if len(elements) == 253:
            primitives.append(a)

    return primitives

if __name__ == "__main__":
    brute_force_primitive()

